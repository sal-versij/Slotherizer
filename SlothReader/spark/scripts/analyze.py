...
def get_prediction(file_name):
    ...
    resp = requests.post(url, headers=headers, params=params,
                         data=open("/usr/share/logstash/csv/photos/" + file_name, "rb").read())
    r = resp.json()
    ...
    for i in range(n_predictions):
        ...
        if probability > 0.60:
            ...
            plt.text(x + w / 2, y, text, fontsize=12.5 / width * height, fontweight="bold", ha="center", va="bottom",
                     fontfamily="monospace", color=color, bbox={"facecolor": "white", "alpha": 0.75, "pad": 2.5})
